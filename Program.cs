        ...
        public void Start()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    Thread thread = new Thread(new ThreadStart(hp.Process));//create thread to run HTTPProcessor in Parallel (Simultaneously)
                    thread.Start();//Start this thread for HTTPProcessor process for each client
                    //hp.Process();
                }
                catch (Exception ex)
                {
