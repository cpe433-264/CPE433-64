...
namespace DNWS
{
    ...
    public class HTTPProcessor
    {
        ...
        public void Process()
        {
            ...
            request = new HTTPRequest(requestStr); //client request for
            ...
            else
            {
                ...
                // local file
                if(!processed) {
                    if (request.Filename.Equals(""))  //no path yet
                    {
        ...
        public void Start()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    HTTPProcessor hp = new HTTPProcessor(clientSocket, _parent); //single threads
                    Thread bua = new Thread(new ThreadStart(hp.Process));
                    //hp.Process(); 
                    bua.Start(); //do threading
                }
                catch (Exception ex)
                {
