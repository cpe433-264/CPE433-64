        ...
        public void Start()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    HTTPProcessor hp = new HTTPProcessor(clientSocket, _parent); // working one thread at the tim

                    // HW 2 //
                    Thread multi = new Thread(new ThreadStart(hp.Process));  // create new thread
                    multi.Start();
